...
saveBtn.addEventListener('click', async () => {
  try {
    ...
    // 设置基础文件夹名称（用于组织文件）
    fileSaver.setBaseFolderName(fileName);
    ...
    // 先保存初始 Markdown 文件
    showStatus('loading', '正在保存 Markdown 文件...');
    const saveResult = await fileSaver.saveMarkdown(markdown, fileName);

    // 等待下载完成
    await fileSaver.waitForDownload(saveResult.downloadId);
    ...
    if (shouldDownloadImages && images && images.length > 0) {
      ...
      // 下载所有图片
      const downloadResults = await fileSaver.downloadImages(images, (completed, total, currentImage, result) => {
        const status = result.success ? '✓' : '✗';
        const errorMsg = result.error ? ` (${result.error})` : '';
        const displayUrl = currentImage.absoluteUrl.length > 50
      ...
      showStatus('loading', '正在更新 Markdown 文件...');
      const updateDownloadId = await fileSaver.updateMarkdown(markdown, fileName);
      await fileSaver.waitForDownload(updateDownloadId);
    }
